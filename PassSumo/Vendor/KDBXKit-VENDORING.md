...
| 2026-09-13 | `Sources/KDBXKit/Extensions/Date+dotnet.swift` | `secondsSinceDotNetEpoch` saturates instead of trapping in `Int64(Double)` when a decoded date is out of `Int64` range (a hostile `Int64.max` timestamp opened fine and made the vault unsavable). Issue #176. **Report upstream.** |
| 2026-09-13 | `Sources/KDBXKit/KDBX/Color.swift` | Malformed `<Color>` returns `nil` instead of `assertionFailure` (Debug abort). Typed-error, never a trap. Issue #176. **Report upstream.** |
| 2026-09-13 | `Sources/KDBXKit/UnlockData.swift` | Hex key-file path uses UTF-8 length, not grapheme count, and `decodeHexKeyFile` returns `nil` instead of `precondition` on a length mismatch. Issue #175/#176. **Report upstream.** |
| 2026-09-13 | `Sources/KDBXKit/KDF/Argon2KDF.swift` | `UInt32(exactly:)` on file-derived iterations/memory before narrowing. Issue #176. **Report upstream.** |
