...
- [A failed e2e wedges the test host](failed-e2e-wedges-the-test-host.md) — two gates guard `make e2e` here and the second needs a person, so an unattended run fails and wedges `testmanagerd` for every session; recovery is SIGKILL of the host daemon only, never `killall`.
